...
def test_log_works(capsys: pytest.CaptureFixture[str], log_level: str) -> None:
...
def test_log_message_is_correct(capsys: pytest.CaptureFixture[str]) -> None:
    log.LogConfig.configure_logging()
    log.info("test")
    captured_log = json.loads(capsys.readouterr().out)
...
def test_log_level_is_uppercase(capsys: pytest.CaptureFixture[str]) -> None:
    log.LogConfig.configure_logging()
    log.info("test")
    captured_log = json.loads(capsys.readouterr().out)
...
def test_log_function_name_is_correct(capsys: pytest.CaptureFixture[str]) -> None:
    log.LogConfig.configure_logging()
    log.info("test")
    captured_log = json.loads(capsys.readouterr().out)
...
def test_log_module_name_is_correct(capsys: pytest.CaptureFixture[str]) -> None:
    log.LogConfig.configure_logging()
    log.info("test")
    captured_log = json.loads(capsys.readouterr().out)
...
def test_log_module_path_is_correct(capsys: pytest.CaptureFixture[str]) -> None:
    log.LogConfig.configure_logging()
    log.info("test")
    captured_log = json.loads(capsys.readouterr().out)
...
def test_log_line_number_is_correct(capsys: pytest.CaptureFixture[str]) -> None:
    log.LogConfig.configure_logging()
    log.info("test")
    current_frame: types.FrameType | None = inspect.currentframe()
    assert current_frame
    log_line_number = inspect.getframeinfo(current_frame).lineno - 1
    captured_log = json.loads(capsys.readouterr().out)
    assert captured_log["line_number"] == log_line_number


# TODO: use freezegun
# def test_log_timestamp_is_correct(capsys: pytest.CaptureFixture[str]) -> None:
#     log.LogConfig.configure_logging()
#     log.info("test")
#     captured_log = json.loads(capsys.readouterr().out)
